 ...
 * @param  {Boolean} options.multiple To reject messages in bulk, clients
 *                                    set the multiple flag
 *                                    @default false
 * @param  {Boolean} options.requeue @default false
 * @return {[type]}         [description]
 */
Message.prototype.reject = function (options) {
  options = options || {};
  var multiple = options.multiple === true;
  var requeue = options.requeue === true;
  this._state = 'REJECTED';
  var self = this;
  return this.channel.getChannel()
    .then(function (channel) {
      return channel.nack(self.rawMessage, multiple, requeue);
    });
};
...
Message.prototype.requeue = function () {
  ...
  return this.channel.getChannel()
    .then(function (channel) {
      return channel.nack(self.rawMessage, requeue);
    });
};
